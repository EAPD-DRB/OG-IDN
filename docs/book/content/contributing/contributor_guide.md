...
7. From your command line or terminal, navigate to your local `OG-IDN` directory.
...
9. Create the project environment, with all of the necessary packages to execute the source code, in a single step:
    ...
    ```
      OG-IDN$ uv sync --extra dev
    ```

    This creates a local `.venv` directory with the `ogidn` package and its development dependencies installed at the exact versions pinned in `uv.lock` (downloading a compatible Python interpreter if needed). For documentation/Jupyter Book work, run `uv sync --extra dev --extra docs` instead.

10. Run commands in the environment by prefixing them with `uv run` (for example, `uv run python examples/run_og_idn.py`), or activate the environment first:

    ```
      OG-IDN$ source .venv/bin/activate
    ```
...
[^recent_python]:`OG-IDN` is currently tested to run on Python 3.12 and 3.13. You do not need to manage the Python version yourself: `uv sync` reads the project's requirements and downloads a compatible interpreter automatically.
